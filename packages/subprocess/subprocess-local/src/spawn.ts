...
export function taskkillProcessTree(pid: number): void {
  ...
  // the console-only taskkill from flashing a window when the host has none.
...
export function spawnSubprocess(spec: SubprocessSpawnSpec, internals: SpawnInternals = {}): LocalSubprocessHandle {
  ...
  const child = spawn(program, args, {
    ...
    // When the host process has no console to share (a GUI/session host),
    // CreateProcess hands the console child a brand-new visible window unless
    // CREATE_NO_WINDOW is set; tool commands must not flash a terminal on the
    // user's desktop. Node ignores this option off Windows.
