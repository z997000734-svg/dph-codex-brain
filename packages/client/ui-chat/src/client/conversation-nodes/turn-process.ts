...
function processSpec(state: TurnProcessState, turn: TurnLocation): TurnProcessSpec | null {
  ...
  // Fold reasoning into the Turn-level process disclosure whenever ANY
  // assistant message of the Turn carries reasoning (not only the final
  // answer), so intermediate "思考" blocks collapse into the single Turn fold.
  const inlineReasoning = turn.steps.some((step) => {
    const data = step.data.get('assistant-step') as Readonly<AssistantChatData> | undefined
    return data?.blocks.some(block => block.kind === 'reasoning' && block.text.trim() !== '') === true
  })
