...
import { CodexSection, type CodexInjected } from './CodexSection.tsx'
import { CodexController } from './codex-store.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['remote.codexBrain'], (codexCtx) => {
    const unwrap = <T,>(result: { ok: true; value: T } | { ok: false; error: { message: string } }): T => {
      if (!result.ok) throw new Error(result.error.message)
      return result.value
    }
    const codex = new CodexController({
      status: async () => unwrap(await codexCtx.remote.codexBrain.status()),
      configuration: async () => unwrap(await codexCtx.remote.codexBrain.configuration()),
      configure: async value => { unwrap(await codexCtx.remote.codexBrain.configure(value)) },
      detect: async () => unwrap(await codexCtx.remote.codexBrain.detect()),
      test: async () => unwrap(await codexCtx.remote.codexBrain.test()),
      pause: async value => { unwrap(await codexCtx.remote.codexBrain.pause(value)) },
      cancel: async id => unwrap(await codexCtx.remote.codexBrain.cancel(id)),
      setDefault: async () => { unwrap(await codexCtx.remote.codexBrain.setDefault()) },
    })
    codexCtx.effect(() => () => codex.dispose(), 'codex management refresh teardown')
    codexCtx.slots.inject('settings.section', () => codexCtx.slots.register({
      name: 'settings.section', id: 'codex', order: 11, label: () => t('codexTitle'),
      inject: (): CodexInjected => ({ controller: codex, hooks: { codex: codex.store }, t }),
    }, CodexSection))
  })
